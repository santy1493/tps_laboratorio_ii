...
namespace Entidades
{
    ...
    public class Mundo : IEstadisticas
    {
        ...
        public string MostrarPorMillonPaises(string nombrePais)
        {
            Pais pais = this.ValidarPais(nombrePais);
            StringBuilder sb = new StringBuilder();
            sb.AppendLine($"{pais.Nombre}\n\n");
            sb.AppendLine($"{pais.CalcularInfectadosPorMillon()}\n\n\n");
            sb.AppendLine($"{pais.CalcularFallecidosPorMillon()}\n\n\n");
            sb.AppendLine($"{pais.CalcularVacunadosPorMillon()}");

            return sb.ToString();
        }

        private static string PorcentajePorMillon(double d1, double d2)
        {
            double porcentaje = d2 * 100 / d1;
        ...
        /// <summary>
        /// Valida dos paises a partir de strings devuelve porcentaje infectados, fallecidos y vacuinados por millon en formato string
        /// </summary>
        /// <param name="paisUnoStr"></param>
        /// <param name="paisDosStr"></param>
        /// <returns></returns>
        public string CompararPaisesPorMillon(string paisUnoStr, string paisDosStr)
        {
            Pais paisUno = this.ValidarPais(paisUnoStr);
            Pais paisDos = this.ValidarPais(paisDosStr);
            StringBuilder sb = new StringBuilder();

            string porcentajeInfectados = PorcentajePorMillon(paisUno.CalcularInfectadosPorMillon(), paisDos.CalcularInfectadosPorMillon());
            string porcentajeFallecidos = PorcentajePorMillon(paisUno.CalcularFallecidosPorMillon(), paisDos.CalcularFallecidosPorMillon());
            string porcentajeVacunados = PorcentajePorMillon(paisUno.CalcularVacunadosPorMillon(), paisDos.CalcularVacunadosPorMillon());

            sb.AppendLine($"{porcentajeInfectados}\n\n\n");
            sb.AppendLine($"{porcentajeFallecidos}\n\n\n");
            sb.AppendLine($"{porcentajeVacunados}");

            return sb.ToString();
        }

        public Pais ValidarPais(string nombrePais)
        {
            foreach(Pais p in this.listaPaises)
            {
                if(p.Nombre == nombrePais)
                {
                    return p;
                }
            }
